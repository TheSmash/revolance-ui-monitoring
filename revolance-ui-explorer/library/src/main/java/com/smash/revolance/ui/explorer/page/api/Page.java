...
public class Page implements IPage
{
    ...
    public void parse( ) throws Exception
    {
        if( !hasBeenParsed() )
        {
            ...
            setWidth(  Integer.parseInt( String.valueOf( (Long) getUser().getBot().runJS( "return Math.max(document.body.clientWidth, window.innerWidth)"  ) ) ) );
            setHeight( Integer.parseInt( String.valueOf( (Long) getUser().getBot().runJS( "return Math.max(document.body.clientHeight, window.innerHeight)" ) ) ) );
...
//            }
    ...
    private List<Element> _getContent() throws Exception
    {
        ...
        if( !isExternal() && !isBroken() )
        {
            ...
            Element.filterElementsIncludedInEachOthers( content, getArea()*0.95, 1/8 );

            setContent( content );

            System.out.println( bean.getContent().size() + " pertinent elements found" );
