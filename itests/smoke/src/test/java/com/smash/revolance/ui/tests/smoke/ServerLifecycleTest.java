...
public class ServerLifecycleTest extends TestConstants
{
    ...
    {
        assertThat( listDistribFiles(), hasItems(endsWith("start.sh"), endsWith("stop.sh"), endsWith("status.sh")));
    }

    public static List<String> listDistribFiles()
    {
        List<String> files = new ArrayList<>();
        List<File> distribFiles = (List<File>) FileUtils.listFiles(DISTRIB_DIR, new String[]{"sh"}, false);
